// ===== Año en footer (todas las páginas) =====
...
// ===== Galería (solo corre si la página tiene #galleryGrid, ej. galeria.html) =====
...
if (galleryGrid) {
  designs.forEach(d => {
    ...
    card.addEventListener('click', () => {
      // Guardamos el estampado elegido y vamos al personalizador en otra página
      sessionStorage.setItem('chosenPrint', d.img);
      sessionStorage.setItem('chosenPrintTitle', d.name);
...
// ===== Personalizador (solo corre si la página tiene #garmentSvg, ej. personalizar.html) =====
...
if (garmentSvg) {
  ...
  function setColor(hex) {
    document.querySelectorAll('.garment-fill').forEach(f => { f.style.fill = hex; });
  }
  ...
  // Si venimos de la galería con un estampado elegido, lo aplicamos; si no, usamos el default
  const incomingPrint = sessionStorage.getItem('chosenPrint');
  if (incomingPrint) {
    printImage.setAttribute('href', incomingPrint);
    printImage.setAttribute('opacity', '1');
    document.querySelectorAll('.print-thumb').forEach(b => b.classList.remove('active'));
    sessionStorage.removeItem('chosenPrint');
    sessionStorage.removeItem('chosenPrintTitle');
  } else {
  ...
  if (orderIdEl) orderIdEl.textContent = String(Math.floor(Math.random() * 900) + 100).padStart(4, '0');
